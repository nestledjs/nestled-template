...
| Emulation          | Starting emulation requires `platform.users.emulate` (or the root super-admin bypass).                                         | An unprivileged account impersonating another user.                                       | Access-policy tests, `auth.resolver.spec.ts`, Doctor          |
...
| RBAC               | Platform and organization policies are declared with scoped decorators; row/object ownership remains a service check.          | Hidden authorization contracts or a capability grant bypassing target ownership.          | Access-policy tests, Doctor `access-policy` checks            |
| RBAC               | Role mutation and delegated platform user administration enforce grant/privilege ceilings.                                     | An administrator creating access above their own or acting on a peer/higher principal.    | Access-control and organization service tests                 |
...
| Generated CRUD     | Every generated CRUD operation is admin-only, and application API code cannot import generated CRUD or its recursive selector. | Generic filters or relation selection becoming a user-accessible database query compiler. | Doctor `admin-crud-boundary` check and ESLint import boundary |
...
- Keep generated CRUD admin-only. `@crudAuth` is forbidden; create a purpose-built resolver and DTO
  for any user-facing workflow.
...
- Keep scope derived from `@CtxUser()` and verified memberships rather than trusting GraphQL input
  IDs.
- Use the scoped RBAC decorators for operation-level capability policy. Continue to verify the
  supplied record belongs to that scope inside the service. See [Scoped RBAC](./scoped-rbac.md).
- Keep `ApiCrudDataAccessService` and generated CRUD DTOs out of application resolvers. The legacy
  public `createSelect` helper must remain deleted. Use explicit Prisma arguments through
