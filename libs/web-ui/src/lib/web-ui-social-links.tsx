...
const linkedinCompanyProfile: Profile = {
  ...
  matches: [
    ...
    {
      match: '(https?://)?(www.)?linkedin.com/mwlite/company/({PROFILE_ID})',
      group: 3,
      type: TYPE_MOBILE,
      pattern: 'https://linkedin.com/mwlite/company/{PROFILE_ID}',
    },
    { match: '({PROFILE_ID})', group: 1 },
    { match: '/company/({PROFILE_ID})', group: 1 },
  ],
}

export function WebUiSocialLinks(props: SocialLinksProps) {
  if (!props?.member) return null
  const socialLinks = new SocialLinks()
  // Add the company profile
  socialLinks.addProfile('linkedin-company', linkedinCompanyProfile.matches)
  ...
  if (!facebookLink && !twitterLink && !instagramLink && !linkedinLink && !youtubeLink) return null
  ...
  return (
    <div className={'flex space-x-6 mt-8 mx-auto w-fit'}>
      ...
      {/*{member?.website ? (*/}
      {/*  <a href={`${standardizeUrl(member?.website ?? '')}`} target="_blank" style={{ color: fill }} rel={'noreferrer'}>*/}
      {/*    <GlobeAltIcon className={'h-6'} style={{ color: fill }} />*/}
      {/*  </a>*/}
      {/*) : null}*/}
    </div>
  )
